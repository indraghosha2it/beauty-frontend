...
import { 
  ...
  Sparkles, ClipboardList, Barcode,
  Scan, Eye, Copy, Check
} from 'lucide-react';
import Script from 'next/script';

export default function ModeratorBarcodeScannerPage() {
  ...
  const generateBarcodeSVG = (barcodeNumber) => {
    if (typeof window !== 'undefined' && window.JsBarcode) {
      try {
        window.JsBarcode("#moderator-scanner-barcode-svg", barcodeNumber, {
          format: "CODE128",
          lineColor: "#000000",
          width: 2,
          height: 50,
  ...
  const fetchProductByBarcode = async (barcodeValue) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Add to scan history (moderator sees less history items)
        setScanHistory(prev => [
          { 
          ...
          ...prev.slice(0, 4) // Limited to 5 items for moderator
        ]);
    ...
    <>
      ...
      <div className="min-h-screen p-6" style={{ backgroundColor: '#FFF9F0' }}>
        <div className="max-w-6xl mx-auto">
                      ...
                      {product.barcode && (
                        <div className="mt-2 pt-2 border-t border-gray-200">
                          ...
                          {product.barcodeImageUrl ? (
                            ...
                            <div className="bg-white p-2 rounded-lg text-center">
                              <svg id="moderator-scanner-barcode-svg" width="200" height="60" className="mx-auto"></svg>
          ...
          {/* Scan History - Limited to 5 items for moderator */}
