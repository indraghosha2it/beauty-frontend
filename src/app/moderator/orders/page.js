...
//                   className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 hover:bg-white transition"
//                 />
//               </div>
//               <div>
...
//           <button onClick={onClose} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 text-sm font-medium transition">
//             Cancel
//           </button>
//           <button onClick={handleSubmit} disabled={loading} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium transition">
...
//             Cancel
//           </button>
//           <button onClick={handleDelete} disabled={loading} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium transition">
...
//   const currentStatusInfo = ORDER_STATUSES.find(s => s.value === selectedStatus);
...
//             <div className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${currentStatusInfo?.color || 'bg-gray-100 text-gray-700'}`}>
//               {currentStatusInfo?.icon && <currentStatusInfo.icon className="w-3 h-3" />}
//               <span>{currentStatusInfo?.label || selectedStatus}</span>
//             </div>
//           </div>
...
//             className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium transition"
//           >
...
//             <div className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${currentPaymentInfo?.color || 'bg-gray-100 text-gray-700'}`}>
...
//               </select>
//             ) : (
//               <div className="px-3 py-2 text-sm bg-gray-100 text-gray-500 rounded-lg">
...
// const OrderDetailsModal = ({ isOpen, onClose, order, onStatusUpdate, onPaymentUpdate, userRole }) => {
...
//               }}
//               className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-all ${statusInfo?.color || 'bg-gray-100 text-gray-700'} ${canChangeStatus ? 'cursor-pointer hover:opacity-80' : 'cursor-default'}`}
//               title={canChangeStatus ? 'Click to update status' : 'Status cannot be changed'}
//             >
//               {statusInfo?.icon && <statusInfo.icon className="w-3 h-3" />}
//               <span>Order: {statusInfo?.label || order.orderStatus}</span>
...
//                 onPaymentUpdate();
//               }}
//               className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-all ${paymentInfo?.color || 'bg-gray-100 text-gray-700'} cursor-pointer hover:opacity-80`}
//               title="Click to update payment status"
//             >
//               <FaMoneyBillWave className="w-3 h-3" />
//               <span>Payment: {paymentInfo?.label || order.paymentStatus}</span>
...
//                   <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium">Subtotal:</td><td className="px-3 py-1.5 text-right">৳{order.subtotal?.toFixed(2)}</td></tr>
//                   <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium">Shipping:</td><td className="px-3 py-1.5 text-right">৳{order.shippingCost?.toFixed(2)}</td></tr>
//                   {order.discount > 0 && (
//                     <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium text-green-600">Discount:</td><td className="px-3 py-1.5 text-right text-green-600">- ৳{order.discount.toFixed(2)}</td></tr>
//                   )}
...
//           <button onClick={() => window.print()} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors flex items-center gap-1.5 text-sm font-medium">
...
// // Main Moderator Orders Page Component
...
//     const placedOrders = ordersList.filter(o => o.orderStatus === 'placed').length;
//     const confirmedOrders = ordersList.filter(o => o.orderStatus === 'confirmed').length;
//     const processingOrders = ordersList.filter(o => o.orderStatus === 'processing').length;
//     const shippedOrders = ordersList.filter(o => o.orderStatus === 'shipped').length;
...
//     const todayOrders = ordersList.filter(o => new Date(o.createdAt) >= today).length;
...
//       processingOrdersCount: processingOrders + shippedOrders
//     };
//   };
...
//       const data = await response.json();
//       if (data.success) {
//         const calculatedStats = calculateStatsFromOrders(data.data);
...
//     if (!statusInfo) return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{status}</span>;
//     return (
...
//     const methods = {
//       'cod': { label: 'COD', color: 'bg-orange-100 text-orange-700', icon: FaMoneyBillWave },
//       'online': { label: 'Online', color: 'bg-blue-100 text-blue-700', icon: FaCreditCard },
//       'bkash': { label: 'bKash', color: 'bg-pink-100 text-pink-700', icon: FaMobileAlt },
//       'nagad': { label: 'Nagad', color: 'bg-purple-100 text-purple-700', icon: FaMobileAlt }
//     };
...
//           <p className="text-xl font-bold text-gray-900">{value?.toLocaleString() || 0}</p>
//         </div>
...
//                 </h1>
//               </div>
//               <p className="text-sm text-gray-500 mt-1 ml-11">Monitor and manage all customer orders</p>
//             </div>
//             <div className="flex items-center gap-3">
//               <button
//                 onClick={exportToCSV}
//                 className="flex items-center gap-2 px-4 py-2.5 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors text-sm font-medium shadow-sm"
//               >
...
//               <StatCard title="Total Orders" value={stats?.totalOrders || 0} icon={<FaBox className="w-5 h-5 text-blue-500" />} color="bg-blue-50" />
//               <StatCard title="Pending Payment" value={stats?.pendingOrders || 0} icon={<FaClock className="w-5 h-5 text-yellow-500" />} color="bg-yellow-50" />
//               <StatCard title="Processing" value={stats?.processingOrdersCount || stats?.processingOrders || 0} icon={<FaSpinner className="w-5 h-5 text-purple-500" />} color="bg-purple-50" />
//               <StatCard title="Delivered" value={stats?.deliveredOrders || 0} icon={<FaCheckCircle className="w-5 h-5 text-green-500" />} color="bg-green-50" />
//               <StatCard title="Cancelled" value={stats?.cancelledOrders || 0} icon={<FaTimesCircle className="w-5 h-5 text-red-500" />} color="bg-red-50" />
//               <StatCard title="Today" value={stats?.todayOrders || 0} icon={<FaCalendarAlt className="w-5 h-5 text-cyan-500" />} color="bg-cyan-50" />
//               <StatCard title="Revenue" value={stats?.totalRevenue || 0} icon={<FaMoneyBillWave className="w-5 h-5 text-emerald-500" />} color="bg-emerald-50" />
//             </div>
//           ) : (
//             <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
//               {[...Array(7)].map((_, i) => (
...
//                   setCurrentPage(1);
//                 }}
//                 className="px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 hover:bg-white transition text-sm"
//               >
...
//                   <tr>
//                     <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Order ID</th>
//                     <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Customer</th>
//                     <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Phone</th>
...
//                     <th className="px-3 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Payment</th>
...
//                         <td className="px-3 py-3 text-xs font-mono font-medium text-gray-900">
//                           {order.orderNumber || order._id.slice(-8).toUpperCase()}
//                         </td>
//                         <td className="px-3 py-3 text-xs">
//                           <div className="font-medium text-gray-900 truncate max-w-[150px]">{order.customerInfo?.fullName}</div>
//                           <div className="text-gray-500 text-xs truncate max-w-[150px]">{order.customerInfo?.email}</div>
//                         </td>
//                         <td className="px-3 py-3 text-xs text-gray-600">{order.customerInfo?.phone}</td>
//                         <td className="px-3 py-3 text-xs text-right font-bold text-blue-600">৳{order.total?.toFixed(2)}</td>
...
//                                 ❌ {new Date(order.cancelledAt).toLocaleDateString('en-BD', {
//                                   day: '2-digit', month: 'short', year: 'numeric'
//                                 })}
//                               </div>
//                             </div>
...
// app/moderator/orders/page.js - BeautyBucket Theme with Invoice Download
'use client';
...
const ORDER_STATUSES = [
  { value: 'placed', label: 'Placed', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaClock, nextStatuses: ['confirmed', 'cancelled'] },
  { value: 'confirmed', label: 'Confirmed', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaCheckCircle, nextStatuses: ['processing', 'cancelled'] },
  { value: 'processing', label: 'Processing', color: 'bg-[#FFF5F6] text-[#8B7A8C] border-[#FFD2DB]', icon: FaSpinner, nextStatuses: ['shipped', 'cancelled'] },
  { value: 'shipped', label: 'Shipped', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaShippingFast, nextStatuses: ['delivered', 'cancelled'] },
...
const SearchableSelect = ({ name, value, onChange, options, placeholder, required, disabled, error }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        className={`w-full px-3 py-2 border rounded-xl focus-within:ring-2 focus-within:ring-[#EE4275] focus-within:border-transparent cursor-pointer flex items-center justify-between transition-all text-sm ${
          ...
          <div className="p-2 border-b border-[#FFD2DB]/30">
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search..."
                className="w-full pl-9 pr-3 py-1.5 border border-[#FFD2DB]/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#EE4275] text-sm"
                onClick={(e) => e.stopPropagation()}
              />
            </div>
          </div>
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option, idx) => (
                <button
                  ...
                  type="button"
                  onClick={() => handleSelect(option)}
                  className="w-full px-4 py-2 text-left hover:bg-[#FFF5F6] transition-colors text-sm text-[#2D1B2E]"
                >
                  {option}
                </button>
              ))
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm overflow-y-auto p-4">
      <motion.div 
        ...
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="relative bg-white rounded-2xl border border-[#FFD2DB]/30 shadow-2xl w-full max-w-2xl my-8 overflow-hidden"
      >
        <div className="p-4 bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white sticky top-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <FaEdit className="w-5 h-5" />
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto">
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Full Name *</label>
                <input
                  type="text"
                  value={formData.fullName}
                  onChange={(e) => setFormData({ ...formData, fullName: e.target.value })}
                  className="w-full px-3 py-2 text-sm border border-[#FFD2DB]/50 rounded-xl focus:ring-2 focus:ring-[#EE4275] focus:border-transparent bg-[#FFF5F6]/20 hover:bg-white transition text-[#2D1B2E]"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Email</label>
                <input
                  type="email"
                  value={formData.email}
                  onChange={(e) => setFormData({ ...formData, email: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Phone *</label>
                <input
                  type="tel"
                  value={formData.phone}
                  onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Division *</label>
                <SearchableSelect
                  name="division"
                  value={formData.division}
                  onChange={handleInputChange}
              ...
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">District/City *</label>
                <SearchableSelect
                  name="city"
                  value={formData.city}
                  onChange={handleInputChange}
              ...
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Upazila/Thana *</label>
                <SearchableSelect
                  name="zone"
                  value={formData.zone}
                  onChange={handleInputChange}
              ...
              <div>
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Union/Area</label>
                <SearchableSelect
                  name="area"
                  value={formData.area}
                  onChange={handleInputChange}
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Address *</label>
                <textarea
                  value={formData.address}
                  onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                  rows="2"
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Delivery Note</label>
                <textarea
                  value={formData.deliveryNote}
                  onChange={(e) => setFormData({ ...formData, deliveryNote: e.target.value })}
                  rows="2"
        ...
        <div className="p-4 border-t border-[#FFD2DB]/30 bg-[#FFF5F6]/20 flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2 border border-[#FFD2DB]/50 text-[#8B7A8C] rounded-xl hover:bg-white transition text-sm font-medium">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={loading} className="flex-1 px-4 py-2 bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white rounded-xl hover:shadow-lg hover:shadow-[#EE4275]/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium">
            {loading ? <FaSpinner className="w-3 h-3 animate-spin" /> : <FaSave className="w-3 h-3" />}
            Save Changes
          </button>
        </div>
...
// Delete Confirm Modal - Beauty Theme
const DeleteConfirmModal = ({ isOpen, onClose, order, onDelete }) => {
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <motion.div 
        ...
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="relative bg-white rounded-2xl border border-[#FFD2DB]/30 shadow-2xl w-full max-w-md overflow-hidden"
      >
        ...
        <div className="p-5 text-center">
          <p className="text-[#2D1B2E] text-sm mb-2">Are you sure you want to delete this order?</p>
          <p className="text-xs text-[#8B7A8C]">Order #{order?.orderNumber || order?._id?.slice(-8).toUpperCase()}</p>
          <p className="text-xs text-red-500 mt-3">⚠️ This action cannot be undone!</p>
        </div>
        ...
        <div className="p-4 border-t border-[#FFD2DB]/30 bg-[#FFF5F6]/20 flex gap-3">
          ...
          <button onClick={handleDelete} disabled={loading} className="flex-1 px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl hover:shadow-lg hover:shadow-red-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium">
            {loading ? <FaSpinner className="w-3 h-3 animate-spin" /> : <FaTrash className="w-3 h-3" />}
            Delete Order
          </button>
        </div>
...
// Status Update Modal - Beauty Theme
const StatusUpdateModal = ({ isOpen, onClose, order, onUpdate }) => {
  const [selectedStatus, setSelectedStatus] = useState('');
  const [trackingNumber, setTrackingNumber] = useState('');
  const [deliveryNote, setDeliveryNote] = useState('');
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto">
          <div>
            <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Current Status</label>
            <div className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border ${currentStatusInfo?.color || 'bg-[#FFF5F6] text-[#8B7A8C] border-[#FFD2DB]'}`}>
              {currentStatusInfo?.icon && <currentStatusInfo.icon className="w-3 h-3" />}
              <span>{currentStatusInfo?.label || selectedStatus}</span>
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Change Status To</label>
            <select
              value={selectedStatus}
              onChange={(e) => {
                setSelectedStatus(e.target.value);
            ...
            {availableNextStatuses.length === 0 && (
              <p className="text-xs text-[#FFC107] mt-1 flex items-center gap-1">
                <FaExclamationTriangle className="w-3 h-3" />
                This order is final and cannot be changed further
              </p>
            )}
          ...
          {isCancelling && (
            <div>
              ...
              <textarea
                ...
                rows="3"
                placeholder="Please provide a reason for cancellation..."
                className="w-full px-3 py-2 text-sm border border-[#FFD2DB]/50 rounded-xl focus:ring-2 focus:ring-[#EE4275] focus:border-transparent bg-[#FFF5F6]/20 hover:bg-white transition text-[#2D1B2E] placeholder:text-[#C4B5C5]"
                required
              />
            </div>
          )}
          ...
          <div>
            <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Delivery Note (Optional)</label>
            <textarea
              value={deliveryNote}
              onChange={(e) => setDeliveryNote(e.target.value)}
              rows="2"
        ...
        <div className="p-4 border-t border-[#FFD2DB]/30 bg-[#FFF5F6]/20 flex gap-3">
          ...
          <button
            onClick={handleSubmit}
            disabled={loading || selectedStatus === order.orderStatus}
            className="flex-1 px-4 py-2 bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white rounded-xl hover:shadow-lg hover:shadow-[#EE4275]/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm font-medium"
          >
            {loading ? <FaSpinner className="w-3 h-3 animate-spin" /> : <FaCheckCircle className="w-3 h-3" />}
            Update Status
          </button>
...
// Payment Status Modal - Beauty Theme
const PaymentStatusModal = ({ isOpen, onClose, order, onUpdate }) => {
  const [selectedStatus, setSelectedStatus] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div className="p-4 space-y-3">
          <div>
            <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Current Payment Status</label>
            <div className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border ${currentPaymentInfo?.color || 'bg-[#FFF5F6] text-[#8B7A8C] border-[#FFD2DB]'}`}>
              <FaMoneyBillWave className="w-3 h-3" />
              <span>{currentPaymentInfo?.label || order?.paymentStatus}</span>
            </div>
            <div className="mt-1 text-xs text-[#8B7A8C]">
              Order Status: <span className="font-medium text-[#2D1B2E]">{order?.orderStatus}</span> | 
              Payment Method: <span className="font-medium text-[#2D1B2E]">{order?.paymentMethod === 'cod' ? 'COD' : 'Online'}</span>
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium text-[#2D1B2E] mb-1">Change Payment Status To</label>
            {canChange ? (
              <select
                value={selectedStatus}
                onChange={(e) => setSelectedStatus(e.target.value)}
              ...
              <div className="px-3 py-2 text-sm bg-[#FFF5F6] text-[#8B7A8C] rounded-xl border border-[#FFD2DB]/50">
                No further changes allowed
              </div>
            )}
          </div>
...
const OrderDetailsModal = ({ isOpen, onClose, order, onStatusUpdate, onPaymentUpdate, onDownloadInvoice, userRole }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm overflow-y-auto p-4">
      <motion.div 
        ...
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: 20 }}
        className="relative bg-white rounded-2xl border border-[#FFD2DB]/30 shadow-2xl w-full max-w-3xl my-8 overflow-hidden"
      >
        <div className="p-4 bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white sticky top-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto">
          <div className="flex flex-wrap gap-2 mb-5">
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-all ${statusInfo?.color || 'bg-[#FFF5F6] text-[#8B7A8C] border-[#FFD2DB]'} ${canChangeStatus ? 'cursor-pointer hover:opacity-80' : 'cursor-default'}`}
              title={canChangeStatus ? 'Click to update status' : 'Status cannot be changed'}
            >
              {statusInfo?.icon && <statusInfo.icon className="w-3 h-3" />}
              <span>Order: {statusInfo?.label || order.orderStatus}</span>
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-all ${paymentInfo?.color || 'bg-[#FFF5F6] text-[#8B7A8C] border-[#FFD2DB]'} cursor-pointer hover:opacity-80`}
              title="Click to update payment status"
            >
              <FaMoneyBillWave className="w-3 h-3" />
              <span>Payment: {paymentInfo?.label || order.paymentStatus}</span>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB] hover:bg-white transition-colors disabled:opacity-50"
            >
              {downloading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          <img 
                            src={item.image || 'https://via.placeholder.com/30'} 
                            alt={item.productName}
                            className="w-8 h-8 rounded-lg object-cover border border-[#FFD2DB]/30"
                ...
                <tfoot className="border-t border-[#FFD2DB]/30">
                  <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium text-[#2D1B2E]">Subtotal:</td><td className="px-3 py-1.5 text-right text-[#2D1B2E]">৳{order.subtotal?.toFixed(2)}</td></tr>
                  <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium text-[#2D1B2E]">Shipping:</td><td className="px-3 py-1.5 text-right text-[#2D1B2E]">৳{order.shippingCost?.toFixed(2)}</td></tr>
                  {order.discount > 0 && (
                    <tr><td colSpan="3" className="px-3 py-1.5 text-right font-medium text-green-600">Discount:</td><td className="px-3 py-1.5 text-right text-green-600">- ৳{order.discount.toFixed(2)}</td></tr>
                  )}
                  <tr className="text-sm font-bold">
        ...
        <div className="p-4 border-t border-[#FFD2DB]/30 bg-[#FFF5F6]/20 flex justify-end gap-2">
          <button onClick={() => window.print()} className="px-4 py-2 border border-[#FFD2DB]/50 text-[#8B7A8C] rounded-xl hover:bg-white transition-colors flex items-center gap-1.5 text-sm font-medium">
            <FaPrint className="w-3.5 h-3.5" /> Print
          </button>
          <button onClick={onClose} className="px-4 py-2 bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white rounded-xl hover:shadow-lg hover:shadow-[#EE4275]/25 transition-all text-sm font-medium">
            Close
          </button>
        </div>
      </motion.div>
...
// Main Moderator Orders Page Component - Beauty Theme
export default function ModeratorOrdersPage() {
  const router = useRouter();
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  const statusTabs = [
    { value: 'all', label: 'All', count: stats?.totalOrders || 0, color: 'bg-[#EE4275]' },
    { value: 'placed', label: 'Placed', count: stats?.placedOrders || 0, color: 'bg-[#EE4275]' },
    { value: 'confirmed', label: 'Confirmed', count: stats?.confirmedOrders || 0, color: 'bg-[#FF6B9D]' },
    { value: 'processing', label: 'Processing', count: stats?.processingOrders || 0, color: 'bg-[#C4B5C5]' },
    { value: 'shipped', label: 'Shipped', count: stats?.shippedOrders || 0, color: 'bg-[#EE4275]' },
    { value: 'delivered', label: 'Delivered', count: stats?.deliveredOrders || 0, color: 'bg-green-500' },
    { value: 'cancelled', label: 'Cancelled', count: stats?.cancelledOrders || 0, color: 'bg-red-500' }
  ];
  ...
  const getStatusBadge = (status) => {
    const statusInfo = ORDER_STATUSES.find(s => s.value === status);
    if (!statusInfo) return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#FFF5F6] text-[#8B7A8C] border border-[#FFD2DB]">{status}</span>;
    return (
  ...
  const getPaymentMethodBadge = (method) => {
    const methods = {
      'cod': { label: 'COD', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaMoneyBillWave },
      'online': { label: 'Online', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaCreditCard },
      'bkash': { label: 'bKash', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaMobileAlt },
      'nagad': { label: 'Nagad', color: 'bg-[#FFF5F6] text-[#EE4275] border-[#FFD2DB]', icon: FaMobileAlt }
    };
  ...
  const StatCard = ({ title, value, icon, color }) => (
    <div className="bg-white rounded-2xl border border-[#FFD2DB]/20 p-4 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-[#8B7A8C] font-medium">{title}</p>
          <p className="text-xl font-bold text-[#2D1B2E]">{value?.toLocaleString() || 0}</p>
        </div>
        <div className={`w-10 h-10 ${color} rounded-xl flex items-center justify-center`}>
          {icon}
        </div>
    ...
    <>
      <div className="min-h-screen bg-[#FFF5F6]/20 pb-12 pt-6">
        <div className="container mx-auto px-4 max-w-7xl">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#8B7A8C] mt-0.5">Monitor and manage all beauty orders</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          {stats ? (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
              <StatCard title="Total Orders" value={stats?.totalOrders || 0} icon={<FaBox className="w-5 h-5 text-[#EE4275]" />} color="bg-[#FFF5F6]" />
              <StatCard title="Pending Payment" value={stats?.pendingOrders || 0} icon={<FaClock className="w-5 h-5 text-[#FFC107]" />} color="bg-[#FFF5F6]" />
              <StatCard title="Processing" value={stats?.processingOrdersCount || stats?.processingOrders || 0} icon={<FaSpinner className="w-5 h-5 text-[#8B7A8C]" />} color="bg-[#FFF5F6]" />
              <StatCard title="Delivered" value={stats?.deliveredOrders || 0} icon={<FaCheckCircle className="w-5 h-5 text-green-500" />} color="bg-green-50" />
              <StatCard title="Cancelled" value={stats?.cancelledOrders || 0} icon={<FaTimesCircle className="w-5 h-5 text-red-500" />} color="bg-red-50" />
              <StatCard title="Today" value={stats?.todayOrders || 0} icon={<FaCalendarAlt className="w-5 h-5 text-[#EE4275]" />} color="bg-[#FFF5F6]" />
              <StatCard title="Revenue" value={stats?.totalRevenue || 0} icon={<FaMoneyBillWave className="w-5 h-5 text-green-500" />} color="bg-green-50" />
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
              {[...Array(7)].map((_, i) => (
                <div key={i} className="bg-white rounded-2xl border border-[#FFD2DB]/20 p-4 shadow-sm animate-pulse">
                  <div className="h-4 bg-[#FFD2DB]/30 rounded w-1/2 mb-2"></div>
                  <div className="h-6 bg-[#FFD2DB]/30 rounded w-3/4"></div>
                </div>
              ))}
            </div>
          )}

          {/* Filters - Beauty Theme */}
          <div className="bg-white rounded-2xl border border-[#FFD2DB]/40 p-4 mb-6 shadow-sm">
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <select
                ...
                className="px-4 py-2.5 border border-[#FFD2DB]/50 rounded-xl focus:ring-2 focus:ring-[#EE4275] focus:border-transparent bg-[#FFF5F6]/20 hover:bg-white transition text-[#2D1B2E] text-sm"
              >
                <option value="">All Order Status</option>
                {ORDER_STATUSES.map(status => (
                  <option key={status.value} value={status.value}>{status.label}</option>
          ...
          {/* Status Tabs - Beauty Theme */}
          <div className="mb-6">
            <div className="flex flex-wrap gap-2 border-b border-[#FFD2DB]/30 pb-2">
              {statusTabs.map((tab) => (
                <button
                  key={tab.value}
                  onClick={() => {
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200 flex items-center gap-2 ${
                    activeStatusTab === tab.value
                      ? 'bg-gradient-to-r from-[#EE4275] to-[#FF6B9D] text-white shadow-lg shadow-[#EE4275]/25'
                      : 'bg-white text-[#8B7A8C] hover:bg-[#FFF5F6] border border-[#FFD2DB]/40'
                  }`}
                >
                  <span className={`w-2 h-2 rounded-full ${tab.color}`}></span>
                  {tab.label}
          ...
          <div className="bg-white rounded-2xl border border-[#FFD2DB]/40 shadow-sm overflow-hidden">
            <div className="w-full overflow-x-visible">
              <table className="w-full min-w-[800px] lg:min-w-full">
                <thead className="bg-[#FFF5F6]/50 border-b border-[#FFD2DB]/30">
                  <tr>
                    <th className="px-3 py-3 text-left text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Order ID</th>
                    <th className="px-3 py-3 text-left text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Customer</th>
                    <th className="px-3 py-3 text-left text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Phone</th>
                    <th className="px-3 py-3 text-right text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Total</th>
                    <th className="px-3 py-3 text-center text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Status</th>
                    <th className="px-3 py-3 text-center text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Payment</th>
                    <th className="px-3 py-3 text-left text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Date</th>
                    <th className="px-3 py-3 text-center text-xs font-semibold text-[#8B7A8C] uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="8" className="px-4 py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-xs text-[#8B7A8C]">Try adjusting your search or filters</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    orders.map((order) => (
                      <tr key={order._id} className="border-b border-[#FFD2DB]/20 hover:bg-[#FFF5F6]/30 transition-colors">
                        <td className="px-3 py-3 text-xs font-mono font-medium text-[#2D1B2E]">
                          {order.orderNumber || order._id.slice(-8).toUpperCase()}
                        </td>
                        <td className="px-3 py-3 text-xs">
                          <div className="font-medium text-[#2D1B2E] truncate max-w-[150px]">{order.customerInfo?.fullName}</div>
                          <div className="text-[#8B7A8C] text-xs truncate max-w-[150px]">{order.customerInfo?.email}</div>
                        </td>
                        <td className="px-3 py-3 text-xs text-[#2D1B2E]">{order.customerInfo?.phone}</td>
                        <td className="px-3 py-3 text-xs text-right font-bold text-[#EE4275]">৳{order.total?.toFixed(2)}</td>
                        <td className="px-3 py-3 text-center">
                          <button onClick={() => { setSelectedOrder(order); setShowStatusModal(true); }} className="hover:opacity-80 transition-opacity">
                            {getStatusBadge(order.orderStatus)}
                          </button>
                        ...
                        <td className="px-3 py-3 text-xs text-[#8B7A8C] whitespace-nowrap">
                          {order.orderStatus === 'delivered' && order.deliveredAt ? (
                            <div className="flex flex-col gap-0.5">
                              <div className="text-[#8B7A8C]">
                                <span className="text-[#C4B5C5] text-[10px]">📦</span> {formatDate(order.createdAt)}
                              </div>
                              <div className="text-green-600 font-medium text-xs">
                                ✅ {new Date(order.deliveredAt).toLocaleDateString('en-BD', {
                                  day: '2-digit', month: 'short', year: 'numeric'
                        ...
                        <td className="px-3 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button 
                              onClick={() => { setSelectedOrder(order); setShowDetailsModal(true); }} 
                              className="p-1.5 text-[#EE4275] hover:bg-[#FFF5F6] rounded-xl transition-colors" 
                              title="View Details"
                            >
                              <FaEye className="w-3.5 h-3.5" />
                            </button>
                            <button
                              ...
                              className="p-1.5 text-[#EE4275] hover:bg-[#FFF5F6] rounded-xl transition-colors disabled:opacity-50" 
            ...
            {totalPages > 1 && (
              <div className="px-4 py-3 border-t border-[#FFD2DB]/30 bg-[#FFF5F6]/20 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-[#8B7A8C]">Need help managing orders?</p>
